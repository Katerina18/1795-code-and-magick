...
window.renderStatistics = function(ctx, players, times) {
  ...
  for (var i = 0; i < players.length; i++) {
    var bar_height = BAR_MAX_HEIGHT * times[i] / maxTime;
    ...
    ctx.fillText(
      ...
      CLOUD_Y + CLOUD_HEIGHT - (GAP + 2 * (FONT_HEIGHT + FONT_GAP) + bar_height)
    );
    ...
    ctx.fillRect(
      CLOUD_X + BAR_GAP/2 + i * (BAR_WIDTH + BAR_GAP),
      CLOUD_Y + CLOUD_HEIGHT - (GAP + FONT_HEIGHT + FONT_GAP + bar_height),
      BAR_WIDTH,
      bar_height
    );
  }
